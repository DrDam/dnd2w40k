...
-- Rendu d'un Header interne au bloc monstre : nom du monstre (niveau 1)
-- ou sous-titre (Actions, Réactions, Traits légendaires...).
local function render_header(el)
...
-- délègue chaque type de bloc au bon renderer, et assemble le tout en
-- une seule chaîne, encadrée par \begin{monsterblock}...\end{monsterblock}.
local function render_monster(div)
  local parts = {"\\begin{monsterblock}"}
  for _, block in ipairs(div.content) do
    if block.t == "Header" then
      table.insert(parts, render_header(block))
    elseif block.t == "Div" and block.classes:includes("statline") then
      table.insert(parts, render_statline(block))
    else
      table.insert(parts, render_generic_block(block))
    end
  end
...
function Div(el)
  ...
  return pandoc.RawBlock("latex", render_monster(el))
end
